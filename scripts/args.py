...
try:
    ee.Initialize(
        project="fiery-cistern-454011-e3"
...
def get_satellite_image(latitude, longitude, date_str, buffer_km=5, days_range=15):
    ...
    Returns:
        ee.Image: The image closest to the specified date with acceptable cloud cover
    ...
    # Define the date range (days_range days before and after the target date)
    ...
    # If still no images, try with even higher threshold
    if collection.size().getInfo() == 0:
        print("  No images with <50% cloud cover found, trying with <80% cloud cover")
        collection = (
            ee.ImageCollection("COPERNICUS/S2_SR")
            .filterDate(start_date_str, end_date_str)
            .filterBounds(region)
    ...
    if collection.size().getInfo() == 0:
        raise Exception(
            f"No Sentinel-2 images found for {date_str} (±{days_range} days)"
        )

    # Convert target date to milliseconds since epoch for Earth Engine
    target_timestamp = int(target_date.timestamp() * 1000)

    # Add a property with the absolute difference between image date and target date
    def add_date_diff(img):
        img_timestamp = img.get("system:time_start")
        diff = ee.Number(img_timestamp).subtract(target_timestamp).abs()
        return img.set("date_diff", diff)

    collection_with_diff = collection.map(add_date_diff)

    # Sort by date difference (closest to target date first)
    sorted_by_date = collection_with_diff.sort("date_diff")

    # Get the closest image
    image = sorted_by_date.first()
    ...
    print(f"  This image is closest to the target date: {date_str}")
...
def extract_burn_severity_polygons(image_path):
    """
    Extract red and orange regions from a burn severity image and return them as polygons.

    Args:
        image_path (str): Path to the burn severity image (PNG file).

    Returns:
        dict: GeoJSON data containing the polygons for red and orange regions.
    """
    # Load the image
    ...
    # Define the RGB values for red and orange in the burn severity image
    red_color = np.array([255, 14, 14])  # RGB for red (very high severity)
    orange_color = np.array([255, 142, 17])  # RGB for orange (high severity)

    # Create masks for red and orange regions
    red_mask = np.all(img_array == red_color, axis=-1)
    orange_mask = np.all(img_array == orange_color, axis=-1)

    # Combine the masks to get all regions of interest (red and orange)
    combined_mask = red_mask | orange_mask
    ...
    for contour in contours:
        ...
        polygon_coords = contour[:, [1, 0]].tolist()  # Swap x and y for GeoJSON format
        ...
        feature = {
            ...
            "properties": {
                "severity": "high_or_very_high",  # Indicate the severity level
            },
        }
...
def analyze_wildfire(
    ...
    try:
        ...
        # Extract burn severity polygons from the burn severity image
        burn_severity_polygons = extract_burn_severity_polygons(burn_severity_path)
        ...
        results = {
            ...
            "nbrStats": nbr_stats,
            "images": {
                "preFireTrueColor": os.path.abspath(pre_fire_path),
                "postFireTrueColor": os.path.abspath(post_fire_path),
